...
class Magestore_RewardPointsReport_Block_Adminhtml_Dashboard_Right_Graph extends Mage_Adminhtml_Block_Dashboard_Graph
{
	...
	public function getChartUrl($directUrl = true){
        ...
        //repare chxr
		if(array_key_exists ('chxr', $params))
			$params['chxr'] = rtrim($params['chxr'], '|');
        //repare scale axis map
		if(array_key_exists ('chds', $params))
			$params['chds'] = rtrim($params['chds'], ',');
    ...
    public function mapAxisRange($index, $attribute){
        ...
        if(count($dataSerial)>0) $maxY = max($dataSerial);
		else $maxY = 0;
        $minY = 0;
        $step = 1;
        if ($maxY > 10) {
