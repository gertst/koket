...
class Magestore_RewardPointsBehavior_Model_Actions_Rate extends Magestore_RewardPoints_Model_Action_Abstract implements Magestore_RewardPoints_Model_Action_Interface {

    public function getActionLabel() {
        return Mage::helper('rewardpointsbehavior')->__('Receive point(s) for rating a product');
    }
    ...
    public function getPointAmount() {
        ...
        $amount = (int) Mage::helper('rewardpointsbehavior')->getRateConfig('rate', $review->getStoreId());
        $amountPerDay = (int) Mage::helper('rewardpointsbehavior')->getRateConfig('rate_limit', $review->getStoreId());
        if (is_numeric($amountPerDay) && $amountPerDay > 0) {
            $amountOfDay = Mage::helper('rewardpointsbehavior')->getAmountofDay('rate', $this->getCustomer()->getId());
