...
class Magestore_RewardPointsReferFriends_Block_Rewardpointsreferfriends extends Mage_Core_Block_Template
{
    ...
    function getShareLink(){
        ...
        else return $this->callHelper()->getPolicyLink();
    }
    /**
     * Get Coupon
     * @return type
    ...
    function strReplace($method='sharing'){
        ...
        if($method!='share') $config = $this->callHelper()->getReferConfig('link_to_share_'.$method);
        else $config = $this->callHelper()->getReferConfig('link_to_share_email');
        if($config == 'policy_link' || !$linkshare) $linkshare = $this->callHelper()->getPolicyLink();
        return str_replace(
                    array(
                '{{store_name}}',
                '{{coupon_share}}',
    ...
    public function getPolicyDescription(){
       ...
       $offer = $this->getSpecialOffer();     
		...
		$message .= $value->getDescriptionInvited()."\n";
                ...
                if($value->getDiscountValue()>0) $message .= "Get ".$discount." discount when purchase products in this event.\n";
            }
          }
