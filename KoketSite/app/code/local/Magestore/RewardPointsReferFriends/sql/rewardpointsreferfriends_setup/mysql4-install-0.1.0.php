...
DROP TABLE IF EXISTS {$this->getTable('rewardpoints_special_refer')};

CREATE TABLE {$this->getTable('rewardpoints_special_refer')}(
...
-- Create table product_label_value cho multistore
 DROP TABLE IF EXISTS {$this->getTable('rewardpoints_special_refer_value')};
   CREATE TABLE {$this->getTable('rewardpoints_special_refer_value')} (
  ...
  INDEX (`store_id`),
  FOREIGN KEY (`special_refer_id`) REFERENCES {$this->getTable('rewardpoints_special_refer')} (`special_refer_id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
-- Create tabel rewardpoints_refer_customer
  DROP TABLE IF EXISTS {$this->getTable('rewardpoints_refer_customer')};
  CREATE TABLE {$this->getTable('rewardpoints_refer_customer')}(
  ...
  PRIMARY KEY (`id`)
   ...
   -- ADD COLUMN `rewardpoints_referal_earn` int(11) NOT NULL default '0',
   -- ADD COLUMN `rewardpoints_invited_discount` decimal(12,4) NOT NULL default '0',
   -- ADD COLUMN `rewardpoints_invited_base_discount` decimal(12,4) NOT NULL default '0',
   -- ADD COLUMN `rewardpoints_refer_customer_id` int(11) NOT NULL default '0';
   UPDATE {$this->getTable('rewardpoints_transaction')} set `action`='referfriends' where `action`='offer' ;
